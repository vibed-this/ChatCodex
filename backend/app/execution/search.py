    ...
    async def glob(self, pattern: str, path: Optional[str] = None) -> dict[str, Any]:
        ...
        try:
            ...
            for f in files:
                ...
                p=Path(f)
                if any(part in SEARCH_EXCLUDED_DIRS for part in p.relative_to(base).parts if part not in {".", ".."}):
                    continue
    ...
    async def grep(self, pattern: str, path: Optional[str] = None, include: Optional[str] = None) -> dict[str, Any]:
        ...
        if rg:
            ...
            try:
                ...
                raise ExecutionError("search_timeout", "rg search exceeded the 30 second resource limit") from exc
            if completed.returncode not in {0, 1}:
                raise ExecutionError("search_error", completed.stderr.strip() or "rg search failed")
            for line in completed.stdout.splitlines():
                match=re.match(r"^(.*?):(\d+):(.*)$", line)
                if not match:
                    continue
                if any(part in SEARCH_EXCLUDED_DIRS for part in Path(match.group(1)).parts):
                    continue
                rows.append((os.path.abspath(match.group(1)), int(match.group(2)), match.group(3)[:500]))
                if len(rows)>=limit:
                    break
        else:
            scanned=0
            ...
            for root, dirs, files in os.walk(walk_roots[0]):
                ...
                for fname in files:
                    ...
                    if target and os.path.abspath(fpath) != os.path.abspath(target):
                        continue
                    if include and not any(fnmatch.fnmatch(fname, inc) for inc in _expand_include(include)):
                        continue
                    try:
