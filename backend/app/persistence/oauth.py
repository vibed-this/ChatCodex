...
class OAuthClientRepository:
    ...
    def get(self, client_id: str) -> dict[str, Any] | None:
        ...
        return (
            value
            if isinstance(value, dict) and value.get("client_id") == client_id
            else None
        )

    def save(self, client: dict[str, Any], max_clients: int) -> None:
        client_id = str(client["client_id"])
        with self.db.conn() as connection:
            rows = connection.execute(
                "SELECT key FROM kv_config WHERE key LIKE 'oauth-client:%' ORDER BY rowid ASC"
            ).fetchall()
