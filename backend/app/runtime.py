...
def create_runtime(base_settings: Settings | None = None) -> Runtime:
    ...
    mcp_auth_mode = {"bearer": "token"}.get(str(override("mcp_auth_mode", settings.mcp_auth_mode)), str(override("mcp_auth_mode", settings.mcp_auth_mode)))
    if mcp_auth_mode not in {"token", "oauth", "both", "noauth"}:
        db.close()
        raise ValueError("CHATCODEX_MCP_AUTH_MODE must be token, oauth, both, or noauth")

    internal_ws_key = override("codex_internal_ws_key", settings.codex_internal_ws_key) or secrets.token_urlsafe(48)
    ...
    settings = Settings(**{**settings.__dict__,
        ...
        "oauth_password": override("oauth_password", settings.oauth_password) or web_access_token,
        "oauth_callback_protection": bool(override("oauth_callback_protection", settings.oauth_callback_protection)),
        "public_url": public_url,
        "public_route_kind": public_route_kind if public_route_kind in {"direct", "cloudflared-try", "cloudflared-named"} else "",
        "tunnel_kind": public_route_kind if public_route_kind in {"direct", "cloudflared-try", "cloudflared-named"} else "",
        "chatgpt_tunnel_enabled": bool(override("chatgpt_tunnel_enabled", settings.chatgpt_tunnel_enabled)),
        "chatgpt_tunnel_id": override("chatgpt_tunnel_id", settings.chatgpt_tunnel_id),
        ...
        "codex_external_ws_url": override("codex_external_ws_url", settings.codex_external_ws_url),
        "codex_external_ws_key": override("codex_external_ws_key", settings.codex_external_ws_key),
        "codex_internal_ws_key": internal_ws_key,
        "codex_release_repo": override("codex_release_repo", settings.codex_release_repo),
        "codex_download_url": override("codex_download_url", settings.codex_download_url),
        "tunnel_client_command": override("tunnel_client_command", settings.tunnel_client_command),
        "tunnel_client_release": override("tunnel_client_release", settings.tunnel_client_release),
        "tunnel_auto_restart": bool(override("tunnel_auto_restart", settings.tunnel_auto_restart)),
    })
    ...
    appserver = AppServerManager(settings, port=int(override("codex_ws_port", settings.codex_ws_port)), auto_restart=bool(override("codex_auto_restart", True)), native=native)
    ...
    return Runtime(settings, db, settings_store, native, auth, web_auth, appserver, tunnels, events, approval, execution, mcp, generated_web_token, generated_mcp_token)
