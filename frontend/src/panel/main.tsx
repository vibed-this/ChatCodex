...
function auditDuration(record: AuditRecord): string { if (!record.active) return record.durationMs.toFixed(1); const elapsed = Math.max(0, Date.now() - new Date(record.timestamp).getTime()); return Math.max(record.durationMs, elapsed).toFixed(1); }
