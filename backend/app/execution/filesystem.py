...
class FilesystemService:
    ...
    async def read(self, filePath: str, offset: Optional[int] = None, limit: Optional[int] = None) -> dict[str, Any]:
        ...
        if _is_binary_file(resolved, sample):
            try:
                ...
                return {"title": _rel_title(resolved), "output": "Binary resource read successfully", "metadata": {"preview": binary_mime, "truncated": False}, "mime": binary_mime, "dataBase64": base64.b64encode(data).decode(), "sizeBytes": size}
            except OSError as exc:
                raise ExecutionError("read_error", str(exc)) from exc
    ...
    async def edit(self, filePath: str, oldString: str, newString: str, replaceAll: bool = False) -> dict[str, Any]:
        ...
        if oldString=="":
            ...
            try:
                _atomic_write_bytes(resolved, _join_bom(new_text, has_bom).encode("utf-8"))
            except Exception as e:
                raise ExecutionError("write_error", str(e))
    ...
    async def delete(self, filePath: str) -> dict[str, Any]:
        resolved = _resolve_absolute(filePath)
        if not os.path.exists(resolved):
            raise ExecutionError("not_found", f"Path {resolved} not found")
        try:
            if os.path.isdir(resolved):
                os.rmdir(resolved)
            else:
            ...
            raise ExecutionError("permission_denied", str(exc)) from exc
        except OSError as exc:
            raise ExecutionError("delete_error", str(exc)) from exc
        return {"title": _rel_title(resolved), "output": "Deleted successfully.", "path": resolved, "deleted": True}
