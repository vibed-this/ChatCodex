...
MAX_LINES = 2000
MAX_BYTES = 50 * 1024
MAX_LINE_FALLBACK = MAX_LINES
MAX_BYTES_FALLBACK = MAX_BYTES
...
def _resolve_absolute(p: str, cwd: str | None = None) -> str:
...
def _atomic_write_bytes(path: str, data: bytes) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.debug(
                "temporary write cleanup failed for %s: %s", temp_path, cleanup_exc
            )
        msg = "write_error"
...
def _indentation_flexible_replacer(content: str, find: str) -> Iterator[str]:
    def remove_indent(t: str) -> str:
        ls = t.split("\n")
        non = [line for line in ls if line.strip()]
        if not non:
            return t
        matches = [re.match(r"^(\s*)", line) for line in non]
        if not all(match is not None for match in matches):
            msg = "failed to determine indentation"
            raise ValueError(msg)
        m = min(len(match.group(1)) for match in matches if match is not None)
        return "\n".join(line[m:] if line.strip() else line for line in ls)
...
def _replace_content(
    ...
    for rep in replacers:
        for search in rep(content, old):
            ...
            if (
                search.strip().__len__()
                > max(len(old.strip()) + 500, len(old.strip()) * 4)
                and len(old_lines) != 1
            ):
                msg = "invalid_edit"
                raise ExecutionError(
...
def _parse_patch(patch_text: str) -> list[dict[str, Any]]:
    ...
    try:
        bi = next(i for i, line in enumerate(lines) if line.strip() == begin)
        ei = next(i for i, line in enumerate(lines) if line.strip() == end)
                    ...
                    chunks.append(
                        {
                            "old_lines": old,
                            "new_lines": new,
                            "change_context": ctx or None,
                            "is_end_of_file": is_eof or None,
                        }
                    )
                else:
                    j += 1
            hunks.append(
                {"type": "update", "path": path, "move_path": move, "chunks": chunks}
            )
            i = j
        else:
            i += 1
...
def _derive_new_contents(
    file_path: str, chunks: list[dict[str, Any]], original_text: str
) -> tuple[str, bool]:
    ...
    for ch in chunks:
        ...
        if ctx:
            ...
            if found == -1:
                msg = f"Failed to find context '{ctx}' in {file_path}"
                raise ValueError(msg)
            idx = found + 1
...
def _tail_output(text: str, max_lines: int, max_bytes: int) -> str:
    lines = text.split("\n")
