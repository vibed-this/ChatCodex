...
class PatchService:
    ...
    async def apply(self, patchText: str) -> dict[str, Any]:
        if not patchText:
            msg = "invalid_patch"
            raise ExecutionError(msg, "patchText is required")
        try:
            hunks = _parse_patch(patchText)
        except Exception as e:
            msg = "invalid_patch"
            raise ExecutionError(msg, f"apply_patch verification failed: {e}")
        if not hunks:
            norm = patchText.replace("\r\n", "\n").replace("\r", "\n").strip()
            if norm == "*** Begin Patch\n*** End Patch":
                msg = "invalid_patch"
                raise ExecutionError(msg, "patch rejected: empty patch")
            msg = "invalid_patch"
            raise ExecutionError(msg, "apply_patch verification failed: no hunks found")
        file_changes = []
        total_diff = ""
        for hunk in hunks:
            ...
            if typ == "add":
                ...
                diff = (
                    ...
                    if body
                    else f"diff --git a/{fpath} b/{fpath}\nnew file mode 100644\n"
                )
                diff = trim_diff(diff)
                total_diff += diff + "\n"
                ...
                if not os.path.exists(fpath) or os.path.isdir(fpath):
                    ...
                    raise ExecutionError(
                        msg,
                        f"apply_patch verification failed: Failed to read file to update: {fpath}",
                    )
                try:
                    ...
                    raise ExecutionError(msg, f"apply_patch verification failed: {e}")
                try:
                    new_text, bom = _derive_new_contents(
                        fpath, hunk.get("chunks", []), orig_text
                    )
                except Exception as e:
                    msg = "invalid_patch"
                    raise ExecutionError(msg, f"apply_patch verification failed: {e}")
                ...
                diff = (
                    "\n".join([f"diff --git a/{fpath} b/{fpath}", *body, ""])
                    if body
                    else ""
                )
                diff = trim_diff(diff)
                ...
                for line in difflib.unified_diff(
                    old_stripped.splitlines(), new_stripped.splitlines(), lineterm=""
                ):
                ...
                file_changes.append(
                    {
                        "filePath": fpath,
                        "oldContent": old_stripped,
                        "newContent": new_stripped,
                        ...
                        "deletions": dels,
                        "bom": bom,
                        "rel": _rel_title(move_path or fpath).replace("\\", "/"),
                    }
                )
            elif typ == "delete":
                if not os.path.exists(fpath):
                    ...
                    raise ExecutionError(
                        msg,
                        f"apply_patch verification failed: Failed to read file to delete: {fpath}",
                    )
                try:
                    with open(fpath, "rb") as fh:
                        raw = fh.read()
        ...
        for path in affected:
            if os.path.exists(path):
                if os.path.isdir(path):
                    ...
                    raise ExecutionError(
                        msg,
                        f"apply_patch verification failed: path is a directory: {path}",
                    )
        ...
        try:
            for ch in file_changes:
                ...
                if typ in {"add", "update"}:
                    atomic_write(
                        ch["filePath"],
                        _join_bom(ch["newContent"], ch["bom"]).encode("utf-8"),
                    )
                elif typ == "move":
                    atomic_write(
                        ch["movePath"],
            ...
            msg = "patch_commit_failed"
            raise ExecutionError(msg, detail) from commit_error
        summary = [
            f"{'A' if c['type'] == 'add' else 'D' if c['type'] == 'delete' else 'M'} {c['rel']}"
        ...
        files = [
            {
                ...
                **({"movePath": c["movePath"]} if c.get("movePath") else {}),
            }
            for c in file_changes
        ]
        return {
            ...
            "files": files,
            "applied": True,
            "fileChanges": [c["rel"] for c in file_changes],
        }
